...
module.exports = async (argv) => {
  program
    ...
    .option(
      "-t, --transition-name <name>",
      "name of gl-transition to use",
      (s) => s,
      "fade"
    )
    .option(
      "-d, --transition-duration <duration>",
      "duration of transition to use in ms",
      (v) => parseInt(v),
      500
    )
    ...
    .option(
      ...
      "format to use for temp frame images",
      /^(raw|png|jpg)$/i,
      "raw"
    )
    .option(
    ...
    .option(
      "-O, --temp-dir <dir>",
      "temporary working directory to store frame data"
    )
    .parse(argv);
  ...
  if (program.transitions) {
    try {
      ...
      console.error(
        `error parsing transitions file "${program.transitions}"`,
        err
      );
      throw err;
    }
